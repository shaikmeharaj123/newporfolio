  ...
  useEffect(() => {
    ...
    const draw = (time) => {
      ...
      particles.forEach((particle) => {
        ...
        // Pulsing alpha
        const pulseAlpha = particle.alpha * (0.8 + 0.2 * Math.sin(time * particle.pulseSpeed + particle.pulse));
        ...
        if (particle.r > 1) {
          ...
          const glowGradient = ctx.createRadialGradient(
            particle.x, particle.y, particle.r * 0.5,
            particle.x, particle.y, particle.r * 2.5
          );
          glowGradient.addColorStop(0, accent.replace(/[\d.]+\)$/, `${pulseAlpha * 0.5})`));
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 110) {
            ...
            ctx.strokeStyle = accent.replace(
              /[\d.]+\)$/,
              `${opacity})`
            );
            ctx.lineWidth = 0.5;
...
function HeroProfileImage({ name, src, mounted }) {
  ...
  return (
    <div
      className={`relative mx-auto lg:mx-0 w-64 h-64 md:w-80 md:h-80 transition-all duration-1000 delay-300 ${
        mounted
    ...
    >
      ...
      <div className="absolute -inset-6 border border-gold/10 rounded-full animate-spin-slow" 
        style={{ animationDuration: '25s', animationDirection: 'reverse' }} 
      />
      <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle,var(--color-glow)_0%,transparent_70%)] blur-xl animate-pulse-glow" />

      <div className="relative w-full h-full rounded-full border border-gold/40 bg-[var(--color-panel)] overflow-hidden shadow-[0_0_70px_var(--color-glow)] group hover:shadow-[0_0_100px_var(--color-glow)] transition-all duration-500">
        {src && !imageFailed ? (
          <img
            src={src}
            alt={name}
          ...
          <div className="w-full h-full grid place-items-center bg-[linear-gradient(135deg,var(--color-panel),var(--color-ink))] relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-t from-gold/10 to-transparent animate-pulse-gold" />
            <span className="font-display text-7xl font-black text-gradient-gold relative z-10 group-hover:scale-110 transition-transform duration-500">
              {initials}
            </span>
          </div>
        )}
  ...
  return (
    <span
      ...
      style={{
        ...
        transform: isExiting ? "translateY(-12px) scale(0.95)" : "translateY(0) scale(1)",
